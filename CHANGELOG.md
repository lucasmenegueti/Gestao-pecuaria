...
- **Aborto** (`admin/aborto.tsx`) — indicador reprodutivo. Grava `herd_events` tipo `ABORTO` com a categoria da **matriz** (vaca/novilha presente no piquete) e quantidade. A matriz continua contada no inventário, mas **prenha que aborta evolui automaticamente na mesma transação** (decisão do Lucas: "se há um aborto, naturalmente há uma vaca solteira"): `VACA PRENHA → VACA SOLTEIRA`, `NOVILHA PRENHA → NOVILHA`, via evento `EVOLUCAO` na convenção do Evoluir (notes `"ORIGEM → DESTINO · aborto"`) — ledger, Ficha do Piquete e scripts de auditoria já entendem. Categoria não-prenha só registra o indicador. Botão ao lado de "Nascimentos" no card expandido do piquete (aba Rebanho), visível só onde há matriz — o check de matriz agora inclui `NOVILHA PRENHA`, que faltava (e `nascimento.tsx` ganhou `NOVILHA PRENHA` em `PARENT_CATEGORIES` pelo mesmo motivo: novilha prenha pare, e sem isso o botão "Nascimentos" apontaria pra uma tela que não acha o piquete).
...
**No ar (OTA, 2026-09-09):** runtime `1.0.1`, android+ios. Production update group `a3e122ce-9e36-4983-87ba-f99332a7ce56`. Web: https://nsa-gestao-pecuaria.netlify.app.
