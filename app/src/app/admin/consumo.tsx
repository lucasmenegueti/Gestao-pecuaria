...
export default function ConsumoScreen() {
      ...
      for (const [cat, qty] of selectedEntries) {
        ...
        await db.runAsync(
          `INSERT INTO herd_events (paddock_id, event_type, category, head_count, notes, date)
           VALUES (?, 'CONSUMO', ?, ?, ?, date('now','localtime'))`,
          [Number(paddockId), cat, qty, notes.trim() || null]
        );
      }
      if (router.canGoBack()) router.back();
      else router.replace('/(tabs)/rebanho');
    } catch (err: any) {
      Alert.alert('Erro', err?.message ?? 'Falha ao registrar consumo.');
    } finally {
  ...
  if (reviewing) {
    return (
      <View style={styles.root}>
        <BrandHeader title="Confirmar consumo" context="Rebanho" onBack={() => setReviewing(false)} />
        <SafeAreaView edges={['bottom']} style={{ flex: 1 }}>
          <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
            <Card borderColor={NSA.warn}>
              ...
              {notes.trim() ? <SummaryRow label="Observação" value={notes.trim()} /> : null}
              <Text style={styles.note}>
                Essas cabeças saem do inventário como abate para consumo próprio. O evento fica guardado pra relatório.
              </Text>
            </Card>
            <Button
              title={submitting ? 'Processando…' : 'Confirmar consumo'}
  ...
  return (
    <View style={styles.root}>
      <BrandHeader title="Registrar consumo" context="Rebanho" onBack={() => router.back()} />
      <KeyboardAvoider>
      <SafeAreaView edges={['bottom']} style={{ flex: 1 }}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          {!params.paddockId && (
          ...
          {paddockId && categories.length > 0 && (
            <>
              ...
              <TextInput
                ...
                placeholder="Ex.: abate pra sede, churrasco da equipe…"
